...
# Applying this to SIH26125

This style reference was extracted from [steep.app](https://steep.app) and is the visual target for `apps/web`. A few translations are needed, since Steep is a marketing site and this project is an internal enterprise console.

## What maps directly

| Steep pattern | SIH26125 usage |
|---|---|
| Floating Product Artifact | Asset cards, chain-of-custody timeline entries, credential status cards |
...
Steep's palette is 97% achromatic with a single warm accent, and the Don'ts forbid introducing new chromatic colors. That rule holds everywhere **except the blocked-transfer state**, which is the demo's climax and must read unmistakably as a refusal.

Rather than importing a red, use the existing **Sienna Brown `#5d2a1a` on Blush Peach `#fbe1d1`** as the blocked-transfer treatment. It is already the system's only chromatic pair, it reads as a warning without breaking the palette, and it earns the "one peach card per page" allowance precisely because the blocked transfer is the single most important thing on screen when it happens.
...
Signifier and Sohne are both commercial (Klim Type Foundry). Unless licenses are available, use the documented substitutes and update this section:
- **Signifier →** Source Serif 4 (Google Fonts, free) or `ui-serif, Georgia`
